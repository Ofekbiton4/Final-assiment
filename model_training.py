...
model = ElasticNet(alpha=0.19, l1_ratio=0.87, random_state=42)
scores = cross_val_score(estimator=model, X=X_train_scaled, y=y_train, scoring='neg_mean_squared_error', cv=10)
...
print('The RMSE: ' + str(np.sqrt(mse)))  # Add closing parenthesis: str(np.sqrt(mse)))
