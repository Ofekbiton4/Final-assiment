...
with open('file_values.txt', 'r') as new_file:
    lines = new_file.readlines()

max_value_Area = float(lines[0].split(':')[1].strip())
min_value_Area = float(lines[1].split(':')[1].strip())
max_value_RoomNumber = float(lines[2].split(':')[1].strip())
min_value_RoomNumber = float(lines[3].split(':')[1].strip())
...
def predict():
    ...
    room_number= ((room_number- min_value_RoomNumber)/(max_value_RoomNumber-min_value_RoomNumber))
    area = float(area)
    area=((area-min_value_Area)/(max_value_Area-min_value_Area))
